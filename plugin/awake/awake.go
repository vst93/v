...
func (a *Awake) printHelp() {
	...
	color.Println("  v awake                      Prevent sleep until interrupted")
	color.Println("  v awake <green>-d</> 30m              Prevent sleep for 30 minutes")
	color.Println("  v awake <green>-d</> 2h               Prevent sleep for 2 hours")
	color.Println()
	color.Println("<fg=magenta;op=bold>Options:</>")
	color.Println("  <green>-d</>, <green>--duration</> <time>  Duration (examples: 30s, 30m, 2h, 1h30m)")
	color.Println("  <green>-h</>                     Show this help")
	color.Println()
	color.Println("<fg=magenta;op=bold>Interactive Controls:</>")
	color.Println("  <green>q</> / <green>Esc</> / <green>Ctrl-C</>  Exit and release sleep lock")
	color.Println()
	color.Println("<fg=magenta;op=bold>Platform Implementation:</>")
	color.Println("  <green>macOS</>     caffeinate (prevents display and system sleep)")
	color.Println("  <green>Linux</>     systemd-inhibit (blocks idle, shutdown, sleep)")
	color.Println("  <green>Windows</>   SetThreadExecutionState (system + display required)")
	color.Println()
	color.Println("<fg=magenta;op=bold>Examples:</>")
	color.Println("  <gray># Keep awake during a long download</>")
	color.Println("  v awake")
	color.Println()
	color.Println("  <gray># Prevent sleep for presentation (90 minutes)</>")
	color.Println("  v awake <green>-d</> 90m")
	color.Println()
	color.Println("  <gray># Quick 30-minute awake session</>")
	color.Println("  v awake <green>-d</> 30m")
	color.Println()
	color.Println("<gray>The UI displays elapsed time and automatic shutdown countdown (if set).</>")
	color.Println("<gray>Tips rotate every 4 seconds to keep the display informative.</>")
	color.Println("<gray>--------------------------------------------------</>")
}
