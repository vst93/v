...
func (s *Save) printHelp() {
	color.Println("<gray>--------------------------------------------------</>")
	color.Printf("<fg=cyan;op=bold>📄 save2file - Save Text to File v%s</>\n\n", s.version)
	color.Println("Write text to a new text file, defaulting to Downloads with a timestamped name.")
	color.Println()
	color.Println("<fg=magenta;op=bold>Usage:</>")
	color.Println(`  v save2file                     Save the clipboard text to ~/Downloads/<timestamp>.txt`)
	color.Println(`  echo "hello" | v save2file       Save piped text`)
	color.Println(`  v save2file "hello world"       Save text passed as an argument`)
	color.Println(`  v save2file -name notes.txt -dir ~/notes   Custom filename and directory`)
	color.Println(`  v save2file -out /tmp/note.txt  Save to a full custom path`)
	color.Println()
	...
	color.Println("  <green>-clip</>          Read input from the clipboard")
	color.Println("  <green>-no-reveal</>     Skip opening the folder after saving")
	color.Println("  <green>-h</>             Show this help")
	color.Println()
	color.Println("<gray>Input priority: pipe > argument > clipboard</>")
	color.Println("<gray>After saving, the file manager reveals the file when supported.</>")
	color.Println("<gray>--------------------------------------------------</>")
}
